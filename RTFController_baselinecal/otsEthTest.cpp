...
int main()
{
    ...
    /*
    const int32_t inA = 0xaaaaaaaa;
    const int32_t inB =	0xcccccccc;
    const int32_t inC =	0xf0f0f0f0;
    const int32_t inD =	0xff00ff00;
    eth.send(52, 0x20000000000 | inA);
    eth.send(56, 0x20000000000 | inB);
    eth.send(60, 0x20000000000 | inC);
    eth.send(64, 0x20000000000 | inD);
    */
    ...
    //set input DACs
    uint32_t dac_start = 1750;
    uint32_t dac_end = 1865;
    uint32_t dac_step = 5;
    for(uint32_t dac_value=dac_start; dac_value<dac_end; dac_value=dac_value+dac_step){
      ...
      {
        eth.send(2, (i << 12) | dac_settings[i]);
        printf("%d  %x\n", i, (i << 12) | dac_settings[i]);
        eth.send(1, 0x1);
        eth.send(1, 0x2);
        usleep(20);
      }

      // reset counters and sleep
      //eth.send(0, 0x8);
      usleep(20);
